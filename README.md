...
<p align="center">
  <strong>Fast, lightweight Jetpack Compose quick-access dock and remote button remapper for Google TV & Android TV, seamlessly integrated with Home Assistant.</strong>
</p>
...
- ⚡ **Floating Quick-Access Dock**:
  ...
  - Interactive popup dialogs for light brightness adjustment (0–100%) and climate controls (temperature & HVAC modes).
  - Pin installed Android TV apps directly alongside Home Assistant entities.

- 🎛️ **Native Remote Button Remapper**:
...
- **UI Framework**: [Jetpack Compose](https://developer.android.com/jetpack/compose) for Leanback / TV.
- **Async & Reactive**: Kotlin Coroutines & `StateFlow` reactive pipelines.
...
- **Storage**: Android Jetpack `EncryptedSharedPreferences` + `Security-Crypto`.
...
```bash
# Clone repository
git clone https://github.com/your-username/HomeKey-TV.git
cd HomeKey-TV
